...
OUT_PROT = "/home/claude/legaia/build/PROT_KR5.DAT"
OUT_EXE = "/home/claude/legaia/build/SCUS_KR5.exe"
...
def main():
    ...
    exe2, hook = hook5.patch_exe(exe, idx_bytes)
    print(f"[5] 훅 {len(hook)}B, 폭테이블 {len(idx_bytes)}개 -> {hook5.WIDTH}px")
    ...
    json.dump({ch: [p, i] for ch, (p, i) in mapping.items()},
              open("/home/claude/legaia/build/kr5_map.json", "w"),
