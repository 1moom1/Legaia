#!/usr/bin/env python3
"""PoC3 — LoadImage 재귀를 피하고 GPU 에 직접 전송한다.
...
def build():
    ...
    # --- 2) GPU 에 직접 전송 (LoadImage 를 쓰지 않는다) ---
    a.ins(LUI("t0", 0x1F80))
    a.ins(ADDIU("t0", "t0", 0x1810))       # t0 = GPU_DATA

    a.ins(LUI("t1", 0xA000))
    a.ins(SW("t1", 0, "t0"))               # GP0 0xA0 : CPU -> VRAM
    ...
    a.ins(ADDIU("t2", "t2", lo16(SRC)))    # 소스 포인터
    a.ins(ADDIU("t3", "zero", WORDS))      # 남은 워드 수

    a.label("L_loop")
    a.ins(LW("t4", 0, "t2"))
    a.ins(NOP())                           # ★ 로드 지연
    a.ins(SW("t4", 0, "t0"))               # GPU_DATA 에 쓰기
    a.ins(ADDIU("t2", "t2", 4))
    a.ins(ADDIU("t3", "t3", -1))
    a.bnez("t3", "L_loop")
    a.ins(NOP())
